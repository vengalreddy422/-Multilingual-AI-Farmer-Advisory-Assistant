...
st.set_page_config(page_title="Kisan Mitra - AI Agricultural System", page_icon="🌾", layout="wide")
init_session()
...
# --- SIDEBAR: DYNAMIC LANGUAGE SWITCHER ---
with st.sidebar:
    st.markdown("### 🌐 భాష / Select Language")
    selected_language = st.radio(
        "Language Selection",
        ["తెలుగు (Telugu)", "English (English)"],
        index=0,
        horizontal=True,
        label_visibility="collapsed"
    )
    lang_key = "te" if "తెలుగు" in selected_language else "en"
    t = get_text(lang_key)
...
# Main Navigation Tabs rendered in the chosen language
tab_labels = [
    t["tabs"]["community"],
    t["tabs"]["crops"],
    t["tabs"]["mandi"],
...
with tabs[3]:
    ...
    text_input = st.chat_input("Ask agronomy question...")
    if text_input:
        ...
        with st.chat_message("assistant"):
            with st.spinner("🌾 Generating advice..."):
                advice = orchestrator.generate_advisory(
                    query=user_query,
                    language="Telugu" if lang_key == "te" else "English",
...
# 6. RADAR
with tabs[5]:
    st.subheader(f"🚨 Community Pest Outbreak Radar ({farmer_location})")
    ...
    if outbreaks:
        for ob in outbreaks:
            with st.container(border=True):
                ...
                st.write(f"🛡️ Action Advisory: {ob['advisory']}")
...
with tabs[6]:
    ...
    with c_ir1:
        ...
        acres_in = st.number_input("Cultivated Area (Acres)", value=farmer_acres, min_value=0.5, step=0.5)
    with c_ir2:
...
# 9. CALENDAR
with tabs[8]:
    st.subheader("📅 Crop Activity Schedule")
...
# 11. SCHEMES
with tabs[10]:
    st.subheader(f"🏛️ Matched Government Schemes ({farmer_name_clean})")
...
# 12. PDF
with tabs[11]:
    st.subheader("📜 Digital Prescription Health Card")
    ...
    with col_pdf2:
        st.markdown("### 🗄️ Past Diagnostic Records")
...
print("✅ Pure Dynamic Language Switcher successfully installed into app.py!")